...
import com.gfs.mobile.core.domain.usecase.milling.*
...
class MillBillingViewModel @Inject constructor(
    ...
    private val getMillPriceUseCase: GetMillPriceUseCase,
    private val getChaffPriceUseCase: GetChaffPriceUseCase,
    private val clearMillBillingCacheUseCase: ClearMillBillingCacheUseCase,
    private val saveMillBillingCacheUseCase: SaveMillBillingCacheUseCase,
    ...
    private fun initAuthorizeUser() {
        viewModelScope.launch {
            getAuthenticationTokenUseCase().collect { result ->
    ...
    private fun initializeMillingPrice() {
        ...
        viewModelScope.launch {
            getMillPriceUseCase().collect { response ->
                when (response) {
    ...
    private fun initializeChaffPrice() {
        viewModelScope.launch {
            getChaffPriceUseCase().collect { response ->
                when (response) {
                    is NetworkResource.Success -> {
                        val data = response.data.data?.get(0)
    ...
    fun proceedToPay() {
        ...
        viewModelScope.launch {
            saveMillBillingCacheUseCase(millTransaction)
        }
    }
    ...
    private fun getCustomers() {
        viewModelScope.launch {
            delay(500)
            getCustomersUseCase(10).collect{ response ->
                when (response) {
                    is NetworkResource.Success -> {
    ...
    private fun searchCustomer(customer: String) {
        viewModelScope.launch {
            delay(500)
            searchCustomerUseCase(customer).collect{ response ->
                when (response) {
                    is NetworkResource.Success -> {
                        val data = response.data.data
    ...
    fun addCustomer(
        ...
        viewModelScope.launch {
            ...
            addCustomerUseCase(customer).collect { response ->
                when (response) {
                    is NetworkResource.Success -> {
                        val data = response.data.data
