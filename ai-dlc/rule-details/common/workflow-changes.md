...
3. **Update Execution Plan**: Add stage to `<module_name>/<module_name>-docs/inception/plans/execution-plan.md` with rationale
4. **Update State**: Mark stage as "PENDING" in `<module_name>/<module_name>-docs/aidlc-state.md`
5. **Execute Stage**: Follow normal stage execution process
6. **Log Change**: Document in `<module_name>/<module_name>-docs/audit.md` with timestamp and reason
...
5. **Update State**: Mark stage as "SKIPPED" in `<module_name>/<module_name>-docs/aidlc-state.md`
6. **Adjust Later Stages**: Note that later stages may need manual setup
7. **Log Change**: Document in `<module_name>/<module_name>-docs/audit.md` with timestamp and reason
...
3. **If Restart Chosen**:
   ...
   - Mark stage as "IN PROGRESS" in `<module_name>/<module_name>-docs/aidlc-state.md`
   - Clear stage completion status
   - Re-execute from beginning
4. **Log Change**: Document reason for restart and what will change in `<module_name>/<module_name>-docs/audit.md`
...
4. **If Confirmed**:
   ...
   - Reset all affected stages in `<module_name>/<module_name>-docs/aidlc-state.md`
   - Clear checkboxes in all affected plan files
   - Return to the stage to restart
   - Re-execute from that point forward
5. **Log Change**: Document full impact and reason for restart in `<module_name>/<module_name>-docs/audit.md`
...
2. **Update Execution Plan**: Change depth level in `<module_name>/<module_name>-docs/inception/plans/execution-plan.md` (originally planned in workflow-planning.md)
...
5. **Log Change**: Document depth change and reason in `<module_name>/<module_name>-docs/audit.md`
...
3. **Update State**: Ensure `<module_name>/<module_name>-docs/aidlc-state.md` reflects current status
4. **Log Pause**: Document pause point in `<module_name>/<module_name>-docs/audit.md`
...
**On Resume**:
1. **Detect Existing Project**: Check for `<module_name>/<module_name>-docs/aidlc-state.md`
...
5. **Log Resume**: Document resume point in `<module_name>/<module_name>-docs/audit.md`
...
3. **Update Unit Artifacts**:
   - Modify `<module_name>/<module_name>-docs/inception/application-design/unit-of-work.md`
   - Update `<module_name>/<module_name>-docs/inception/application-design/unit-of-work-dependency.md`
   - Revise `<module_name>/<module_name>-docs/inception/application-design/unit-of-work-story-map.md`
...
2. **Update All Tracking**: Keep `<module_name>/<module_name>-docs/aidlc-state.md`, plan files, and `<module_name>/<module_name>-docs/audit.md` in sync
...
3. **Log Completely**: Document full change history in `<module_name>/<module_name>-docs/audit.md`
