...
5. **Document**: Log the error and resolution in `<module_name>/<module_name>-docs/audit.md`
...
**Error**: Existing `<module_name>/<module_name>-docs/aidlc-state.md` is corrupted
...
**Scenario**: `<module_name>/<module_name>-docs/aidlc-state.md` is corrupted or inconsistent

**Recovery Steps**:
1. Create backup: `<module_name>/<module_name>-docs/aidlc-state.md.backup`
...
5. Document the gap in `<module_name>/<module_name>-docs/audit.md`
...
3. Reset stage status in `<module_name>/<module_name>-docs/aidlc-state.md`
...
2. Document skip reason in `<module_name>/<module_name>-docs/audit.md`
3. Mark stage as "SKIPPED" in `<module_name>/<module_name>-docs/aidlc-state.md`
...
- **Solution**: 
  ...
  2. Check if stage was marked complete in `<module_name>/<module_name>-docs/aidlc-state.md`
...
**Error**: `<module_name>/<module_name>-docs/aidlc-state.md` shows stage complete but artifacts don't exist
- **Cause**: State file updated but artifact generation failed
- **Solution**:
  1. Mark stage as incomplete in `<module_name>/<module_name>-docs/aidlc-state.md`
...
**Error**: Artifacts exist but `<module_name>/<module_name>-docs/aidlc-state.md` shows stage incomplete
- **Cause**: Artifact generation succeeded but state update failed
- **Solution**:
  1. Verify artifacts are complete and valid
  2. Update `<module_name>/<module_name>-docs/aidlc-state.md` to mark stage complete
  3. Proceed to next stage
- **Recovery**: Update state file to reflect actual completion

**Error**: Multiple stages marked as "current" in `<module_name>/<module_name>-docs/aidlc-state.md`
- **Cause**: State file corruption, manual editing
- **Solution**:
  1. Review artifacts to determine actual progress
  2. Ask user which stage they're actually on
  3. Correct `<module_name>/<module_name>-docs/aidlc-state.md` to show single current stage
...
1. **Always validate state**: Check `<module_name>/<module_name>-docs/aidlc-state.md` matches actual artifacts
