...
**Conditional Enforcement**: Extensions may be conditionally enabled/disabled. See `inception/requirements-analysis.md` for the opt-in mechanism. Before enforcing any extension at ANY stage, check its `Enabled` status in `<module_name>/<module_name>-docs/aidlc-state.md` under `## Extension Configuration`. Skip disabled extensions and log the skip in `<module_name>/<module_name>-docs/audit.md`. Default to enforced if no configuration exists. 
...
1. **MANDATORY**: Log initial user request in `<module_name>/<module_name>-docs/audit.md` with complete raw input
2. Load all steps from `inception/workspace-detection.md`
3. Execute workspace detection:
   - Check for existing `<module_name>/<module_name>-docs/aidlc-state.md` (resume if found)
   - Scan workspace for existing code
   - Determine if brownfield or greenfield
   - Check for existing reverse engineering artifacts
4. Determine next phase: Reverse Engineering (if brownfield and no artifacts) OR Requirements Analysis
5. **MANDATORY**: Log findings in `<module_name>/<module_name>-docs/audit.md`
...
**Execution**:
1. **MANDATORY**: Log start of reverse engineering in `<module_name>/<module_name>-docs/audit.md`
...
5. **MANDATORY**: Log user's response in `<module_name>/<module_name>-docs/audit.md` with complete raw input
...
**Execution**:
1. **MANDATORY**: Log any user input during this phase in `<module_name>/<module_name>-docs/audit.md`
...
6. **MANDATORY**: Log user's response in `<module_name>/<module_name>-docs/audit.md` with complete raw input
...
10. **MANDATORY**: Log user's response in `<module_name>/<module_name>-docs/audit.md` with complete raw input
...
8. **MANDATORY**: Log user's response in `<module_name>/<module_name>-docs/audit.md` with complete raw input
...
- **Progress Tracking**: Update `<module_name>/<module_name>-docs/aidlc-state.md` with executed and skipped stages
- **Complete Audit Trail**: Log ALL user inputs and AI responses in `<module_name>/<module_name>-docs/audit.md` with timestamps
...
- **Stage-Level**: Track overall workflow progress in `<module_name>/<module_name>-docs/aidlc-state.md`
...
├── <module-name>/                  # Specific module (e.g., app, core, ui)
│   ├── <module-name>-docs/         # 📄 DOCUMENTATION ONLY (Per-module planning)
...
│   │   ├── aidlc-state.md          # Stage-level tracking for this module
│   │   └── audit.md                # Audit trail for this module
│   └── [module-code-structure]     # Module-specific code
```

**CRITICAL RULE**:
- Application code: Inside module directories (NEVER in <module-name>-docs/)
- Documentation: <module-name>-docs/ only
- Project structure: Per-module documentation allocation
